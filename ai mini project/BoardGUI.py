...
class BoardGUI():
    ...
    # Disables the mouse click event handler.
    def resumeGUI(self):
        self.c.bind("<Button-1>", self.processClick)
    ...
    # Verifies if the clicked checker belongs to the current player based on the game logic.
    # callback function that process user's mouse clicks
    def processClick(self, event):
        col = int(event.x // self.col_width)
        row = int(event.y // self.row_height)
    # Converts the mouse click coordinates (event.x, event.y) into row and column indices.
